...
int exec_elf(unsigned int *eip, unsigned long *page, struct m_inode *inode, char **argv, char **envp) {
    Elf32_Ehdr ehdr;
    Elf32_Phdr phdr;
...
restart_interp:
	...
	if (!(bh = bread(inode->i_dev,inode->i_zone[0]))) {//读取可执行文件的数据块
		retval = -EACCES;
		goto exec_error2;
	}
	ex = *((struct exec *) bh->b_data);	/* read exec-header */
	if (ex.a_magic == ELF_MAGIC) { // elf
        brelse(bh);
        exec_elf(eip, page, inode, argv, envp);
    }
	if ((bh->b_data[0] == '#') && (bh->b_data[1] == '!') && (!sh_bang)) {//sh类型脚本
		/*
...
/* OK, This is the point of no return */
	...
	eip[0] = ex.a_entry;		/* eip, magic happens :-) */
	eip[3] = p;			/* stack pointer */
	unsigned long begin = current->start_code & 0xfffff000;//加载程序起始页面的起始地址
	unsigned long end = (begin + current->brk) & 0xfffff000;//加载程序最后的页面的起始地址
	unsigned long addr;
