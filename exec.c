...
int do_elf(unsigned long * eip,long tmp,char * filename,
	char ** argv, char ** envp)
{
...
restart_interp:
	...
	ehdr = *((Elf32_Ehdr *) bh->b_data);	/* read exec-header */	
	if (!sh_bang) {
...
/* OK, This is the point of no return */
	...
	current->used_math = 0;
	p += change_ldt(text_len,page)-MAX_ARG_PAGES*PAGE_SIZE;
	p = (unsigned long) create_tables((char *)p,argc,envc);
	current->brk = bss_len +
		(current->end_data = data_len +
		(current->end_code = text_len));
	...
	eip[0] = entry_addr;		/* eip, magic happens :-) */
...
restart_interp:
	...
	if (N_MAGIC(ex) == ELF_MAGIC)//这里只检查了魔数，还应该检查其他信息
		do_elf(eip, tmp, filename, argv, envp);
