...
void do_no_page(unsigned long error_code, unsigned long address)
{
    ...
    /* remember that 1 block is used for header when a.out */
    // if elf32, header size is only 52 b, instead of 1kb of a.out
    ...
    if (nsize == 52 && *(unsigned long *)ehdr.ident == 0x464c457f) // if elf
    {
        for (i = 0; i < ehdr.phdr_num; ++i) // read programme header table
        {
